...
if __name__ == "__main__":
    ...
    if os.path.exists("recite_audio.txt"):
        os.remove("recite_audio.txt")
        print('text file removed')

    File = open("recite_audio.txt","w+")
    File.close()
    ...
    while True:
        ...
        if len(audio_list_pos) != 0:
            if len(audio_list_pos) > 1: 
                Listword = list(map(lambda x:reciteList[x-1],audio_list_pos))
                STR = ' and '.join(Listword)
            else:
                STR = reciteList[audio_list_pos[0]-1]
            File = open("recite_audio.txt","a+")
            File.write(STR)
            File.write('\n')
            File.close()
